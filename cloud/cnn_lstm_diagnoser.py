...
class cnn_lstm_mode_detector(nn.Module):
    def __init__(self, x_size, \
                 cnn_feature_map, cnn_kernel_size, \
                 num_layers, hidden_size, dropout, \
                 fc_size, mode_size):
        super(cnn_lstm_mode_detector, self).__init__()
        self.x_size = x_size
        ...
        # RNN
        self.rnn = nn.LSTM(input_size=cnn_feature_map[-1], hidden_size=hidden_size, num_layers=num_layers, batch_first=True, dropout=dropout) # (batch, seq, feature).
...
class cnn_lstm_pf_isolator(nn.Module):
    def __init__(self, x_size, \
                 cnn_feature_map, cnn_kernel_size, \
                 num_layers, hidden_size, dropout, \
                 fc_size, pf_size):
        super(cnn_lstm_pf_isolator, self).__init__()
        self.x_size = x_size
        self.cnn_feature_map = cnn_feature_map
        self.cnn_kernel_size = cnn_kernel_size
        self.num_layers = num_layers
...
class cnn_lstm_pf_identifier(nn.Module):
    def __init__(self, x_size, \
                 cnn_feature_map, cnn_kernel_size, \
                 num_layers, hidden_size, dropout, \
                 fc_size):
        super(cnn_lstm_pf_identifier, self).__init__()
        self.x_size = x_size
        self.cnn_feature_map = cnn_feature_map
        self.cnn_kernel_size = cnn_kernel_size
        self.num_layers = num_layers
