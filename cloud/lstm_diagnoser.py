...
class lstm_mode_detector(nn.Module):
    def __init__(self, x_size, mode_size, rnn_size, fc_size=[], dropout=0.5):
        ...
        '''
        super(lstm_mode_detector, self).__init__()
        self.x_size = x_size
        ...
        self.rnn = nn.LSTM(input_size=x_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True, dropout=dropout) # (batch, seq, feature).
...
class lstm_para_fault_isolator(nn.Module):
    def __init__(self, x_size, para_size, rnn_size, fc_size=[], dropout=0.5):
        ...
        Args:
            ...
            fc_size: a list, the size of the i_th fc module.
                ...
                If fc is not empty, the module will add extral layers.
        '''
        super(lstm_para_fault_isolator, self).__init__()
        self.x_size = x_size
    ...
    def forward(self, x):
        # RNN/GRU
        paras, _ = self.rnn(x) # not the real hidden_states, but the output of LSTM
        # Para Faults
        for l, a in zip(self.fc, self.ac):
            paras = l(paras)
            paras = a(paras)
...
class lstm_para_fault_identifier(nn.Module):
    def __init__(self, x_size, rnn_size, fc_size=[], dropout=0.5):
        ...
        Args:
            ...
            fc_size: a list, the size of the i_th fc module.
                ...
                If fc is not empty, the module will add extral layers.
        '''
        super(lstm_para_fault_identifier, self).__init__()
        self.x_size = x_size
        self.rnn_size = rnn_size
        self.fc_size = fc_size
        hidden_size, num_layers = rnn_size
