...
def sample_data(data_mana, batch, normal_proportion, snr_or_pro, mask, obs):
...
def train(save_path, model_name, model_type, epoch, batch, normal_proportion, data_mana, diagnoser, optimizer, obs_snr, mask, obs, use_cuda):
    train_loss = []
    running_loss = 0
    ...
    for i in range(epoch):
        ...
        x, m, fp_mode, fp_value = sample_data(data_mana, batch, normal_proportion=normal_proportion, snr_or_pro=obs_snr, mask=mask, obs=obs)
        m = m%3
...
def get_cnn_gru_model(t, use_cuda=True):
    if t=='detector':
        model = cnn_gru_mode_detector( x_size=5,\
                           cnn_feature_map=[16, 32, 64, 64], cnn_kernel_size=[8, 4, 4, 4],\
                           num_layers=4, hidden_size=64, dropout=0.5, \
...
if __name__ == "__main__":
    ...
    # train
    train_loss = train(save_path, model_name, args.type, epoch, batch, normal_proportion, data_mana, model, optimizer, obs_snr, mask, obs, use_cuda)
