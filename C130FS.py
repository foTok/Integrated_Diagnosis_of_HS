...
class C130FS:
    ...
    def plot_Z(self, data): # important interface
        ...
        ax_lst[2, 1].plot(x, data[:, 5])
        ax_lst[2, 1].set_xlabel('Time/s')
        ax_lst[2, 1].set_ylabel('z_R')
        plt.show()

    def plot_paras(self, data): # importance interface
        # pumps
        fig, ax_lst = plt.subplots(3, 2)  # A figure with a 3x2 grid of Axes
        fig.suptitle('Fault parameters')  # Add a title so we know which it is
        x = np.arange(len(data))*self.step_len
        ax_lst[0, 0].plot(x, data[:, 0])
        ax_lst[0, 0].set_ylabel('leak_1')
        ax_lst[1, 0].plot(x, data[:, 1])
        ax_lst[1, 0].set_ylabel('leak_2')
        ax_lst[2, 0].plot(x, data[:, 4])
        ax_lst[2, 0].set_xlabel('Time/s')
        ax_lst[2, 0].set_ylabel('leak_L')
        ax_lst[0, 1].plot(x, data[:, 2])
        ax_lst[0, 1].set_ylabel('leak_2')
        ax_lst[1, 1].plot(x, data[:, 3])
        ax_lst[1, 1].set_ylabel('leak_3')
        ax_lst[2, 1].plot(x, data[:, 5])
        ax_lst[2, 1].set_xlabel('Time/s')
        ax_lst[2, 1].set_ylabel('leak_R')
        plt.show()
